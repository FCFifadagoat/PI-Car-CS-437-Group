...
SCALE = 2 # pixel grid ratio (testing)
...
def renderingGridCV():
    # create a space to draw the grid upon
    img = np.zeros((MAP_SIZE * SCALE, MAP_SIZE * SCALE, 3), dtype = np.uint8)

    # map the grid values 
    obstacle_mask = np.repeat(np.repeat(grid, SCALE, axis=0), SCALE, axis = 1)

    img[obstacle_mask == 0] = (0, 80, 0)  # green is for obstacle free spaces
    img[obstacle_mask == 1] = (0, 0, 255)  # red is for where obstacles are present

    
    grid_color = (40, 40, 40)
    for i in range(0, MAP_SIZE * SCALE, SCALE * 10):
        cv2.line(img, (i, 0), (i, MAP_SIZE * SCALE), grid_color, 1)
        cv2.line(img, (0, i), (MAP_SIZE * SCALE, i), grid_color, 1)


    # set the location of the car 
    car_length = 23
    car_width =  15
    x1 = (origin_x - car_length // 2) * SCALE
    y1 = (origin_y - car_width // 2) * SCALE
    x2 = (origin_x + car_length // 2) * SCALE
    y2 = (origin_y + car_width // 2) * SCALE
    cv2.rectangle(img, (x1, y1), (x2, y2), (255, 100, 0), -1) # blue car

    cv2.imshow("Obstacle Mapping", img)
    cv2.waitKey(1)
